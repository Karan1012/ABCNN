...
def train(lr, w, l2_reg, epoch, batch_size, model_type, num_layers, data_type, word2vec, num_classes=2):
    ...
    if data_type == "WikiQA":
        optimizer = tf.compat.v1.train.AdamOptimizer(name="Adam").minimize(model.cost)
    else:
        optimizer = tf.compat.v1.train.AdagradOptimizer(lr, name="optimizer").minimize(model.cost)
