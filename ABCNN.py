...
class ABCNN():
    def __init__(self, s, w, l2_reg, model_type, num_features, d0=300, di=50, num_classes=2, num_layers=2):
        ...
        self.x1 = tf.compat.v1.placeholder(tf.float32, shape=[None, d0, s], name="x1")
        self.x2 = tf.compat.v1.placeholder(tf.float32, shape=[None, d0, s], name="x2")
        self.y = tf.compat.v1.placeholder(tf.int32, shape=[None], name="y")
        self.features =tf.compat.v1.placeholder(tf.float32, shape=[None, num_features], name="features")
        ...
        def make_attention_mat(x1, x2):
            ...
            euclidean = tf.sqrt(tf.reduce_sum(tf.square(x1 - tf.linalg.matrix_transpose(x2)), axis=1))
        ...
        def w_pool(variable_scope, x, attention):
            ...
            with tf.compat.v1.variable_scope(variable_scope + "-w_pool"):
        ...
        def all_pool(variable_scope, x):
            with tf.compat.v1.variable_scope(variable_scope + "-all_pool"):
            ...
            with tf.compat.v1.variable_scope(variable_scope):
                if model_type == "ABCNN1" or model_type == "ABCNN3":
                    with tf.name_scope("att_mat"):
                        aW = tf.compat.v1.get_variable(name="aW",
                                             shape=(s, d),
                        ...
                        x1_a = tf.expand_dims(tf.linalg.matrix_transpose(tf.einsum("ijk,kl->ijl", att_mat, aW)), -1)
                        x2_a = tf.expand_dims(tf.linalg.matrix_transpose(
                            tf.einsum("ijk,kl->ijl", tf.linalg.matrix_transpose(att_mat), aW)), -1)
        ...
        with tf.compat.v1.variable_scope("output-layer"):
